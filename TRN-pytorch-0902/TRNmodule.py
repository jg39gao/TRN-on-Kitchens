...
class RelationModule(torch.nn.Module):
    ...
    def __init__(self, img_feature_dim, num_frames, num_class):
        '''PARAMS:
            img_feature_dim: () @GJJ
            num_frames: = num_frames_relation = num of segmentation
            num_class
        '''
...
class RelationModuleMultiScale(torch.nn.Module):
    ...
    def __init__(self, img_feature_dim, num_frames, num_class):
        ...
        self.subsample_num = 3 # no more than 3 frames selected to sum up
        self.img_feature_dim = img_feature_dim
        self.scales = [i for i in range(num_frames, 1, -1)] #[ n,n-1, ...,3,2 ] generate the multiple frame relations
        ...
        self.num_class = num_class
        self.num_frames = num_frames
        num_bottleneck = 256  # @GJJ why only half of TRN: to reduce the computational cost 
    ...
    def forward(self, input):
        ''' input: a tensor with sizeof (batch_size, num_frames, img_feature_dim)
        '''
    ...
    def return_relationset(self, num_frames, num_frames_relation):
        '''return the realation conbinations( comprised of num_frames) from num_frames frames
...
def return_TRN(relation_type, img_feature_dim, num_frames, num_class):
    ...
    elif relation_type == 'MTRN' or relation_type == 'TRNmultiscale' :
...
if __name__ == "__main__":
    ...
    print(output)
    print(input_var.dtype) #torch.float32
