...
def return_epic(modality):
    noun_classes= pd.read_csv('epic_kitchens/EPIC_noun_classes.csv', header= 0)
    verb_classes= pd.read_csv('epic_kitchens/EPIC_verb_classes.csv', header= 0)
    
    filename_categories=[8,331] #[verb_classes,noun_classes]
    root_data= '/data/acq18jg/epic/frames_rgb_flow'  ## the data source 
    prefix= 'frame_{:010d}.jpg'

    filename_imglist_train='epic_kitchens/train.pkl'#train_02.pkl
    filename_imglist_val='epic_kitchens/val.pkl'#val_02.pkl
    filename_imglist_test='epic_kitchens/test.pkl'
    return filename_categories[0], filename_imglist_train, filename_imglist_val, root_data, prefix

def return_epic_test(modality):
    noun_classes= pd.read_csv('epic_kitchens/EPIC_noun_classes.csv', header= 0)
    verb_classes= pd.read_csv('epic_kitchens/EPIC_verb_classes.csv', header= 0)
    ...
    filename_imglist_train='epic_kitchens/train_02.pkl'#train_02.pkl
    filename_imglist_val='epic_kitchens/val.pkl'#val_02.pkl
    filename_imglist_test='epic_kitchens/test.pkl'
    return filename_categories[0], filename_imglist_train, filename_imglist_val, root_data, prefix
...
def return_dataset(dataset, modality):
    dict_single = {'jester': return_jester, 'something': return_something, 'somethingv2': return_somethingv2,
                   'charades': return_charades, 'moments': return_moments, 'epic': return_epic,'epic_test': return_epic_test}
    if dataset in dict_single:
        file_categories, file_imglist_train, file_imglist_val, root_data, prefix = dict_single[dataset](modality)
    else:
        raise ValueError('Unknown dataset '+dataset)
        
    if 'epic' not in dataset:
