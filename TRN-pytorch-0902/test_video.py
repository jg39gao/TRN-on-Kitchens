...
parser.add_argument('--dataset', type=str, default='moments',
                    choices=['something', 'jester', 'moments', 'somethingv2','epic'])
...
# Initialize frame transforms.  n*Image.Image-> torch(C,H,W)
...
# ---core process :from frames-> prediction-------------------------------
# Make video prediction.
data = transform(frames) # torch.Size([240, 299, 299])
Jprint('frames:{} * {}'.format(len(frames), frames[0].size ), ' transformed into:',data.size())
input = data.view(-1, 3, data.size(1), data.size(2)).unsqueeze(0).cuda()
Jprint('input:',input.size()) #torch.Size([1, 80, 3, 299, 299]) (1, C/3, 3, H, W)
#---way1 to parameters----
print('*'*20,'TSN:')
Tools.parameter_desc(net.base_model)
policy=net.get_optim_policies()
#----way2 to parameters---
toal_params=0 
for p in policy:
    print('-'*20,'{} ( num: {})'.format(p['name'],len(p['params'])),'-'*20,)
    for param in p['params']:
        toal_params+= param.size().numel()
        print(param.size(), param.size().numel())
print('*'*20, 'total parameters: {:,}'.format(toal_params))
#-------
