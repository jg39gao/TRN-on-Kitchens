...
weight_path = './model/bn_inception-9f5701afb96c8044.pth'
weight_fixed_path = './model/bn_inception-9f5701afb96c8044_fixed.pth'
...
def fix_weight(weight_path, weight_fixed_path, weight_filter=''):
    ''' fix the error: 
            RuntimeError: Error(s) in loading state_dict for InceptionV3: size mismatch for conv_batchnorm.weight: copying a param with shape torch.Size([1, 32]) from checkpoint, the shape in current model is torch.Size([32]).
    '''
    state_dict = torch.load(weight_path)
    for name, weights in state_dict.items():
        if len(weights.size())==2 and weights.size(0)==1:#weight_filter in name :
            state_dict[name] = weights.squeeze(0)
    torch.save(state_dict,weight_fixed_path)
    return weight_fixed_path
...
class BNInception(nn.Module):
    def __init__(self, model_path='model_zoo/bninception/bn_inception.yaml', num_classes=101,
        ...
        if re.compile(r'https?://').search(weight_url):
...
class InceptionV3(BNInception):
    weight_fixed_path= fix_weight(weight_path= './model/inceptionv3-cuhk-0e09b300b493bc74c.pth', 
               weight_fixed_path= './model/inceptionv3-cuhk-0e09b300b493bc74c_fixed.pth')
    
    def __init__(self, model_path='model_zoo/bninception/inceptionv3.yaml', num_classes=101,
                 weight_url= weight_fixed_path# 'https://yjxiong.blob.core.windows.net/models/inceptionv3-cuhk-0e09b300b493bc74c.pth'
                 ):
